...
import { Search, Eye, Trash2, AlertTriangle, ChevronLeft, ChevronRight, Briefcase } from 'lucide-react';
...
const PlacedStudentsList = () => {
    ...
    const fetchStudents = async () => {
        try {
            ...
            const res = await axios.get(`http://localhost:5000/api/admin/placed-students`, {
                params: { page, search },
    ...
    const handleDelete = async () => {
        ...
        try {
            const token = localStorage.getItem('token');
            await axios.delete(`http://localhost:5000/api/admin/placed-students/${deleteId}`, {
                headers: { Authorization: `Bearer ${token}` }
            });
            fetchStudents();
            setDeleteId(null);
                    ...
                    <table className="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody className="bg-white divide-y divide-gray-200">
                            {loading ? (
                                ...
                                <tr>
                                    <td colSpan="4" className="px-6 py-10 text-center text-gray-500">No placed students found</td>
                                </tr>
                            ) : (
                                students.map((student) => (
                                    <tr key={student._id} className="hover:bg-gray-50">
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap text-sm font-medium space-x-3">
                                            <button
                                                onClick={() => navigate(`/admin/placed-students/${student.userId?._id}`)}
